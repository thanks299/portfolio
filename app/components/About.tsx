...
import { Code, Book, Music, Camera, Coffee, ClubIcon as Football } from 'lucide-react'
...
export default function About({ setActiveSection }: AboutProps) {
  ...
  // Reset and reload skills animation when 'skills' tab is clicked
  useEffect(() => {
    if (activeTab === 'skills') {
  ...
  const interests = [
    { icon: <Code className="w-6 h-6" />, text: 'Coding' },
    { icon: <Book className="w-6 h-6" />, text: 'Reading' },
    { icon: <Music className="w-6 h-6" />, text: 'Music' },
    { icon: <Camera className="w-6 h-6" />, text: 'Photography' },
  ...
  return (
    <section id="about" className="py-20 bg-gray-50 dark:bg-gray-800 animate-slide-in">
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-4xl mx-auto">
              ...
              {skills.map((skill) => (
                <div key={skill.name} className="mb-4">
                  <div className="flex justify-between mb-1">
                    <span className="text-base font-medium text-blue-700 dark:text-blue-300">{skill.name}</span>
          ...
          {activeTab === 'experience' && (
            <div className="relative border-l border-gray-200 dark:border-gray-700 animate-slide-in">
              {experiences.map((exp, index) => (
                <div key={index} className="mb-10 ml-4">
                  <div className="absolute w-3 h-3 bg-gray-200 rounded-full mt-1.5 -left-1.5 border border-white dark:border-gray-900 dark:bg-gray-700"></div>
                  <time className="mb-1 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">{exp.year}</time>
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{exp.event}</h3>
                </div>
              ))}
            </div>
          )}
          ...
          {activeTab === 'interests' && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 animate-slide-in">
              {interests.map((interest, index) => (
                <div key={index} className="flex items-center space-x-2 bg-white dark:bg-gray-700 p-4 rounded-lg shadow-md">
                  {interest.icon}
